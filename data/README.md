...
| **Our own per-clip scores** | Ours to license, but they are *keyed by CompSpoofV2 clip identifiers* and are a derived work of a CC BY-NC corpus. | **Shipped** in `../scores/` under CC BY-NC 4.0 (`LICENSE-DATA.md`), with CompSpoofV2 attribution required. |
| **Derived metadata** (`derived/`) | Anonymous cluster integers and native audio format per evaluation clip, derived from the corpus. | **Shipped** under CC BY-NC 4.0 (`LICENSE-DATA.md`). See `derived/README.md`. |
