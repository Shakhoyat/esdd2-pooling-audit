...
# "Definition" must be listed here, or "Definition 3" leaves a bare 3 in the scan and
# direction (b) reports an equation-numbering artefact as an unguarded claim.
FIGTAB_REF = re.compile(r"(?:Fig\.|Figure|Table|Theorem|Lemma|Proposition|Definition|Eq\.|Section)\s?\d+")
...
def _is_rounding_of_listed(tok: str, listed: set[str]) -> bool:
    """True if tok is some listed value printed at a coarser precision, or vice versa."""
    try:
        v = float(tok)
    except ValueError:
        return False
    ...
    for l in listed:
        ...
        try:
            ...
            continue
        ldec = len(l.split(".")[1])
        if ldec > dec and f"{w:.{dec}f}" == tok:
            return True
        if dec > ldec and f"{v:.{ldec}f}" == l:
            return True
    return False
...
def main() -> int:
    ...
    spec = doc["numbers"]
    literature = doc.get("meta", {}).get("literature_literals", [])
    tex = a.tex.read_text() if a.tex.exists() else ""
    fracs = source_fractions(tex)
    raw = pdf_text(a.pdf)
    body = body_text(raw, "ABSTRACT")
    ...
    for lit in doc.get("meta", {}).get("figure_literals", []):
        listed.add(lit["token"])          # axis tick labels and structural digits, declared in the YAML
    ...
    for m in re.finditer(r"(?<![\w.])(\d+\.\d+|\d{1,3}(?:,\d{3})+|\d+)(?![\w.])", scan):
        ...
        if r0 <= m.start() < r1 and explained_by_fractions(tok, fracs):
            continue
        # A value the paper also prints at a coarser precision is not unguarded: "0.40" is
        # listed 0.4000 rounded. The test is that rounding, NOT a string prefix -- a prefix
        # rule let any decimal through as soon as a bare "0" or "0.5" was listed anywhere,
        # which hid every new 0.x literal from this direction.
        if "." in tok and _is_rounding_of_listed(tok, listed):
            continue
