...
# The word is banned because an earlier draft called OUR split the development split; every
# analysis in this paper is on the evaluation split. One use survives that ban and is listed here
# with its reason, so the guard keeps its force: any NEW occurrence fails until someone adds it.
ALLOWED_DEVELOPMENT = (
    "estimating this threshold on a development split does not generalize reliably",
)


def test_no_development_split(tex):
    hits = [i + 1 for i, l in enumerate(tex.splitlines())
            if re.search(r"\bdevelopment\b", l, re.I)
            and not any(a in l for a in ALLOWED_DEVELOPMENT)]
    assert not hits, (f"'development' on lines {hits}: the paper's split is the evaluation split, "
                      "and an earlier draft mislabelled it. If the use is a generic one about "
                      "what a submitter might do, add the phrase to ALLOWED_DEVELOPMENT above "
                      "with its reason.")


def test_the_development_allowance_is_still_used(tex):
    """An allowance that no longer matches is a stale excuse; delete it."""
    stale = [a for a in ALLOWED_DEVELOPMENT if a not in tex]
    assert not stale, f"ALLOWED_DEVELOPMENT entries the paper no longer contains: {stale}"
...
def pdf(tmp_path_factory):
    ...
    for f in ("main.tex", "spconf.sty", "fig_identified.pdf", "fig_acds_mechanism.pdf",
              "fig_accurve.pdf"):
