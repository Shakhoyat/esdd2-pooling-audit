...
Tier 0 (CPU, no training) covers everything except the supplementary consistency
experiment (scripts/consistency_experiment/), which requires retraining two heads and is tier 1.
"""
...
from .acds_artefacts import acds_gate, acds_table
...
def table2(n_draws: int = 2000, seed: int = 1337) -> dict:
    ...
    for axis in ("env", "speech"):
        def cells(rows):
            ...
            # `cond` drops the gated-in structural zeros before scoring (no
            # include_inapplicable=True), an older convention superseded by
            # acds_gate()'s eer_gated (src/pooling_audit/acds_artefacts.py), which
            # is the one the paper's Section 4.5 / Table "endtoend" actually prints
            # (0.2028/0.1892, not this function's 0.2022/0.1886). Kept for the
            # audit trail -- see verify/VERIFICATION_REPORT.md, "old convention
            # conflict" -- not the convention to use for any new printed claim.
...
def s3_gates_and_sweep() -> dict:
    ...
    # The same submitted scores read as probabilities rather than logits: c = 0 is then the
    # bottom of the range instead of an interior point, so the atom lands on the other side
    # of the threshold. Section 4.2 prints both.
    prob_env = 1.0 / (1.0 + np.exp(-s_env))
    out = dict(c0=fill(0.0), c05=fill(0.5), oracle=fill(0.5),
               c0_probability=component_eer(labels, np.where(labels == 0, 0.0, prob_env),
                                            "env", include_inapplicable=True),
...
def all_artefacts(n_draws: int = 2000, seed: int = 1337) -> dict:
    ...
    acds = acds_table()          # Table 1's twenty cells, the crossings, the two identities
    acdsg = acds_gate()          # Section 4.5: lambda, rho, the gated EERs and the errors
