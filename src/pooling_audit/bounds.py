...
def pairwise_determined(published: list[float], w0: float) -> dict:
    ...
    return dict(n_systems=len(published), n_pairs=len(pairs), pair_size=2,
                ...
                # the two the paper and Fig. 1 print: how little the best-placed published
                # system is pinned down, and how little the worst is
                narrowest=min(hi - lo for lo, hi in iv),
                widest=max(hi - lo for lo, hi in iv),
                intervals=iv)
