"""Section 4.3's train/score consistency experiment, recomputed from saved scores.
...
configurations x 2 arms x 3 seeds) ship in scores/consistency/, so the four
numbers Section 4.3 reports are recomputable on CPU.
...
def _run(labels: np.ndarray, s: np.ndarray, axis: str) -> dict:
    ...
    # Proposition 3's fixed point, FAR = wa * FRR_a, on this arm's own curves.
    grid = np.unique(-s)
